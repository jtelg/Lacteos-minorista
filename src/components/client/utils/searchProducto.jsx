...
class SearchProducto extends React.Component {
  ...
  componentDidMount() {
    APIConsultas.picadas.GET().then(async (dataprod) => {
  ...
  search(ev) {
    ...
    this.setState({
      prodFilter: this.state.productos.filter((v) => {
        if (q) {
          if (
            v.nombre?.toLowerCase().indexOf(q.toLowerCase()) > -1 ||
            v.ingredientes?.toLowerCase().indexOf(q.toLowerCase()) > -1
          ) {
            this.setState({ visible: true });
            return true;
          }
